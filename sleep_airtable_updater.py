...
if not API_KEY:
    print("ERROR: AIRTABLE_API_KEY not set. Check your .env file.", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    end_filter = None
    if args.start_date:
        start_filter = datetime.strptime(args.start_date, "%Y-%m-%d").date()
    if args.end_date:
        end_filter = datetime.strptime(args.end_date, "%Y-%m-%d").date()
